...
if __name__ == "__main__":
    # Generate some random data for testing (e.g., 3 dimensions, 10000 samples)
    data = np.random.multivariate_normal([1, 2, 3], np.eye(3), size=10000).T
    mn = MultiNormal(data)
    
    # Test the pdf method
    x = np.array([[1], [2], [3]])  # Ensure x is shaped as (d, 1)
    pdf_value = mn.pdf(x)
    print("PDF value for x = [[1], [2], [3]]:", pdf_value)
